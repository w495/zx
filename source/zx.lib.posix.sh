...
__zx__usage() {
  ...
  __zx -ti '| -t? | --te= |   --emphasis= |      --opt_text-effect= |'
...
{ # __zx__opt_text_effect__*
  __zx__opt_text_effect__code() {
    case $(awk -vi="${1}" 'BEGIN{$0=X;print tolower(i)}') in
    0 | n | clear | reset)
      __zx__opt_text_effect__code__=0
      ;;
    1 | b | bold)
      __zx__opt_text_effect__code__=1
      ;;
    2 | d | f | dim | faint)
      __zx__opt_text_effect__code__=2
      ;;
    3 | i | italic)
      __zx__opt_text_effect__code__=3
      ;;
    4 | u | underline)
      __zx__opt_text_effect__code__=4
      ;;
    5 | l | blink)
      __zx__opt_text_effect__code__=5
      ;;
    7 | r | reverse)
      __zx__opt_text_effect__code__=7
      ;;
    8 | c | conceal)
      __zx__opt_text_effect__code__=8
      ;;
    9 | s | x | strike* | del)
      __zx__opt_text_effect__code__=9
      ;;
    '')
      __zx__opt_text_effect__code__=-1
      ;;
  ...
  __zx__opt_text_effect__code_seq() {
    opt_te_name_seq="${1}"
    ...
    __zx_punct_to_sep "${opt_te_name_seq}" "${te_sep}"
    opt_te_name_seq="${__zx_punct_to_sep__}"

    opt_te_name_seq="${opt_te_name_seq}${te_sep}"
    while test "${opt_te_name_seq#*"${te_sep}"}" != "${opt_te_name_seq}"; do
      te_name="${opt_te_name_seq%%"${te_sep}"*}"
      if test -n "${te_name}"; then
        __zx__opt_text_effect__code "${te_name}"
        if test "${__zx__opt_text_effect__code__}" -gt 0; then
          if test -n "${te_code_seq}"; then
            te_code_seq="${te_code_seq}${te_sep}"
          fi
          te_code_seq="${te_code_seq}${__zx__opt_text_effect__code__}"
        elif test "${__zx__opt_text_effect__code__}" -eq -2; then
          opt_te_name_seq=$(
            echo "${opt_te_name_seq}" | sed "s/./${te_sep}&/g"
          )
        fi
      fi
      opt_te_name_seq="${opt_te_name_seq#*"${te_sep}"}"
    done
    __zx__opt_text_effect__code_seq__="${te_code_seq}"

    unset -v __zx__opt_text_effect__code__
    unset -v __zx_punct_to_sep__

    unset -v opt_te_name_seq
    unset -v te_sep
    unset -v te_code_seq
    unset -v te_name
  }
...
__zx__code_str() {
  ...
  if test -n "${opt_fg_name}"; then
    __zx__color__std_name "${opt_fg_name}"
    __zx__color__fg_code "${__zx__color__std_name__}"
    code_seq="${__zx__color__fg_code__}"
  fi
  if test -n "${opt_bg_name}"; then
    __zx__color__std_name "${opt_bg_name}"
  ...
  if test -n "${opt_te_name_seq}"; then
    __zx__opt_text_effect__code_seq "${opt_te_name_seq}" "${fmt_sep}"
    code_seq="${code_seq}${fmt_sep}${__zx__opt_text_effect__code_seq__}"
  fi
...
__zx__tail() {
  __zx__opt_text_effect__code reset
  # COMPATIBILITY NOTE:
  # ---------------------------------------------------------------
  #   bash/zsh:
  #     ESC = \x1b = \e = \E
  ...
  __zx__tail__="\0033[${__zx__opt_text_effect__code__}m"
}
...
__zx__opt() {
  ...
  __zx__opt_seq__=$(getopt -n "${nm}" -o "${so}" -l "${lo}" -- "${@}")
  eval set -- "${__zx__opt_seq__}"
  unset nm so lo

  while test -n "${__zx__opt_seq__}"; do
    case ${1} in
    # MAIN opt_seq
...
__zx__clean() {
  ...
  unset __zx__tail
  unset __zx__opt_text_effect__code__
  unset __zx__opt_text_effect__code_seq
}
